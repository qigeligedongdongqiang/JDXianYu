...
NSString * const JDSuccessScanQRCodeNotification = @"JDSuccessScanQRCodeNotification";
NSString * const JDScanQRCodeMessageKey = @"JDScanQRCodeMessageKey";

@interface JDScanView()<AVCaptureMetadataOutputObjectsDelegate>
...
+ (instancetype)scanViewShowInController: (UIViewController *)controller
{
    ...
    JDScanView * scanView = [[JDScanView alloc] initWithFrame:SCREENBOUNDS];
    
    if ([controller conformsToProtocol: @protocol(JDScanViewDelegate)]) {
        scanView.delegate = (UIViewController<JDScanViewDelegate> *)controller;
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    if (metadataObjects.count > 0) {
        ...
        if ([self.delegate respondsToSelector: @selector(scanView:codeInfo:)]) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName: JDSuccessScanQRCodeNotification object: self userInfo: @{ JDScanQRCodeMessageKey: metadataObject.stringValue }];
        }
    }
}
